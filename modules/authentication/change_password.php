...
$userid=$_SESSION['user2'];
if(isset($_POST['submit_pass']))
{
	$currPass = md5(mysql_real_escape_string($_POST['currentPassword']));
	$newPass =md5( mysql_real_escape_string($_POST['newPassword']));
	$confrPass = md5(mysql_real_escape_string($_POST['confirmPassword']));
	$message='';
	$error_message='';
	$result=mysql_query("SELECT * FROM users WHERE id_number='$userid'")or die(mysql_error());;
	...
	if($row['password']==$currPass)
	{
		if($confrPass==$newPass)
		{
			...
			{
				mysql_query("UPDATE `users` SET `password`='$newPass' WHERE `id_number`='$row[id_number]'");
				 $message.= "<div class='alert bg-success alert-dismissable'>
										...
										You have successfully Changed Your Password. Keep it safe! <br>Your next login will require the new password!</center> <a href='#' class='alert-link'></a>
									</div>";
			}
			else
			{
		 	...
		 	$message.="<div class='alert alert-danger alert-dismissable'>
				...
				$message.="<div class='alert alert-danger alert-dismissable'>
										<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><center>
										Current Password is Wrong! Please try again.</center><a href='#' class='alert-link'></a>.
									</div>";}
}
?>
<!DOCTYPE html>
...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		<div class="row">
			<ol class="breadcrumb">
				...
				<li class="active"><a href="../profile/index.php">Profile</a></li><li class="active">change Password</li>
			</ol>
		</div><!--/.row-->
  ...
  <form name="frmChange" method="post" action=""  accept-charset="UTF-8" role="form" id="login-form" >
  ...
  <form method="post">
      <fieldset>
        <div class="form-group input-group">
          ...
          <input type="password" name="currentPassword"  class="form-control" placeholder="Current Password" required="" autofocus=""/>
        </div>
        <div class="form-group input-group">
          <span class="input-group-addon">
            <i class="glyphicon glyphicon-lock">
        ...
        <div class="form-group">
          ...
          <br>
           <a href="../profile/index.php"> <button type="submit" name="cancel" value="Submit" class="btn btn-danger btn-block">
            Cancel
          </button></a>
        </div>
      </fieldset>
